import {escapeHTML as e,safeURL} from './lib.mjs?v=1.3.0';
...
export function fieldHome(data,cards,context='',focus=''){
  const practical=data.articles.filter(a=>a.mode==='practical');
  const entries=[
    ['company','准备见装修公司','先讲清荟雅苑旧合租房和半包意向，再核人工辅材、拆旧和项目经理的承诺。','带走：面谈问题和工地观察项'],
  ...
  return `<section class="hero"><div class="hero-copy"><span class="eyebrow">荟雅苑 · 104㎡旧房半包准备</span><h1>先把这套房，<br><em>一步步装明白。</em></h1><p>原4位自如租客，计划10月17日之前交付，目前倾向半包。先弄清拆旧范围、团队分工、两卫与窗边需求，再决定怎么施工和采购。</p><div class="hero-actions"><a class="primary" href="#house">看我家的实用攻略 ↗</a><a class="secondary" href="#project">核对我家需求</a></div></div><figure class="hero-visual"><img src="assets/blender-renders/overall.jpg?v=3.1.4" alt="本项目现有原木方案模型，非实测施工图"><figcaption>围绕你家的三房两卫、飘窗与七字餐边柜</figcaption></figure></section>${context}${focus}<div class="stat-row"><div><strong>${practical.length}</strong><span>实战专题</span></div><div><strong>${data.articles.length}</strong><span>全部知识条目</span></div><div><strong>40</strong><span>本地资料已梳理</span></div><div><strong>${e(data.updated)}</strong><span>本次更新</span></div></div><details class="learning-details"><summary>遇到具体问题，再查通用实战手册</summary><div class="section-head"><div><span class="eyebrow">WHAT ARE YOU DOING NEXT?</span><h2>你现在正要做什么？</h2></div></div><div class="scenario-grid">${entries.map(([id,title,desc,result],i)=>`<a class="scenario-card" href="#field/${id}"><span class="scenario-number">0${i+1}</span><h3>${e(title)} ↗</h3><p>${e(desc)}</p><small>${e(result)}</small></a>`).join('')}</div></details><section class="project-panel"><div><span class="eyebrow">YOUR HOME, YOUR PRIORITIES</span><h2>你家优先确认的几件事</h2><ul><li>先测临路噪音与窗边使用，再定门窗和飘窗台面。</li><li>七字餐边柜、电器与餐桌一起放样，别只看柜子立面。</li><li>三房两卫全屋翻新，把两卫洁具与厨房设备提前选型。</li></ul><a href="#article/start-project-risk">查看本项目约束 ↗</a></div><div><h3>涉及拆墙的部分要单独核查</h3><p>厨房拓宽推拉门、阳台改门窗、飘窗改造仍是条件方案。先确认结构、物业与相关专业要求，再下单或开工。本文不会把模型当成施工依据。</p><a href="#article/process-demolition-structure">拆改边界与核查方法 ↗</a></div></section><details class="learning-details"><summary>我想从头学：按装修阶段查看</summary><div class="roadmap">${data.stages.map((s,i)=>`<a class="stage" href="#category/${s.category}"><span class="stage-num">0${i+1}</span><h3>${e(s.title)} ↗</h3><p>${e(s.deliverable)}</p><small>${e(s.hold)}</small></a>`).join('')}</div></details><div class="field-links"><a href="#tools/compare">团队比较表 ↗</a><a href="#tools/budget">装修总账 ↗</a><a href="#tools/templates">现场记录模板 ↗</a><a href="#study">这份资料包怎么用 ↗</a></div>`;
}
