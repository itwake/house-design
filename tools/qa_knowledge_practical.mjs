...
try{
 for(const width of [1440,390,320]){
  ...
  for(const route of ['#home','#field','#cases','#study','#sources']){await visit(route);await fit(route);check(width+' '+route+' h1',await page.locator('#view h1').count()===1);}
  ...
  await visit('#home');await page.screenshot({path:'tmp/knowledge-qa/practical-home-'+width+'.png',fullPage:true});
  await context.close();
 }
