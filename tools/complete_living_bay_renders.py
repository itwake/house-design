            ...
            manifest.setdefault('renderedViews', {})[view] = {
                **before,
                'retainedFrom': {
                    ...
                    'reason': '本轮仅移除客厅窗前家具；此未改房间或不含窗前桌椅的视角沿用原图，未重新计算间接照明。'
                }
            }
            retained.append(view)
        manifest['renderInheritance'] = {
            'reviewedAt': '3.4.1', 'baselineCommit': BASE,
            'currentViews': fresh, 'referenceViews': retained,
            'note': '客厅相关四视角已重渲；保留图有原图哈希、相机及旧模型来源，不伪造为当前模型新帧。'
        }
        path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
        print(f"PASS {scheme['id']}: {len(fresh)} current frames, {len(retained)} explicitly versioned reference frames; no stale living-bay desk view.")
